 ...
 *   BillQuery
 */
function queueTransaction(modelName, method, filter) {
  ...
  switch(method) {
    case 'get':
      inputXMLDoc = builder.create('QBXML', {version: '1.0'})
        ...
        .ele(modelName+'QueryRq', {'requestID': '100'})
        .ele('MaxReturned')
        .text('0');
      break;
    case 'post':
  ...
  if (inputXMLDoc) strRequestXML = inputXMLDoc.end({'pretty': false});
  request.push(strRequestXML);

  return request;
}

function processQueuedTransaction(){
...
qbws.QBWebConnectorSvc.QBWebConnectorSvcSoap.receiveResponseXML = function (args) {
  ...
  to_json(hresult, function (error, data) {
