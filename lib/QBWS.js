...
function queueTransaction(transaction) {
  ...
  switch(transaction.method) {
    case 'get':
      ...
      inputXMLDoc = builder.create('QBXML', {version: '1.0'})
        ...
        .ele(transaction.modelName.capitalize()+'QueryRq', {'requestID': (queuedTransactions.length + 1).toString()})
        .ele('OwnerID')
        .text('0');
      break;
    case 'post':
  ...
  if (inputXMLDoc) {
    ...
    queuedTransactions.push(strRequestXML);
  }
}
...
qbws.QBWebConnectorSvc.QBWebConnectorSvcSoap.authenticate = function (args) {
  ...
  serviceLog('    args.strUserName: ' + args.strUserName.trim())
  serviceLog('    args.strPassword: ' + args.strPassword.trim())
...
qbws.QBWebConnectorSvc.QBWebConnectorSvcSoap.receiveResponseXML = function (args) {
  ...
  if (objectNotEmpty(hresult)) {
    ...
    percentage = counter * 100 / queuedTransactions.length;
    if (percentage >= 100) {
      counter = 0;
    }
  ...
  to_json(response, function (error, data) {
    ...
    console.log(data.QBXML.QBXMLMsgsRs);
...
qbws.QBWebConnectorSvc.QBWebConnectorSvcSoap.sendRequestXML = function (args) {
  ...
  total = queuedTransactions.length;

  if (counter < total) {
    request = queuedTransactions[counter];
