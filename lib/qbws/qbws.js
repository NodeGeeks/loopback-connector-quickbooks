...
class QBWebService {
  ...
  reSync() {
    ...
    //http.get({host: '192.168.0.2', path: 'qbwc://docontrol/UpdateNow?AppName=CNS%20Steel%20Sales'}, function (response) {
  ...
  queueTransaction(transaction) {
    ...
    if (!Model) {
      if (transaction.modelName == 'Employee') {
        Model = app.loopback.createModel(transaction.modelName, {}, {base: 'User'});
      } else {
        Model = app.loopback.createModel(transaction.modelName, {}, {
          base: 'PersistedModel', acls: [{
            "accessType": "*",
            "principalType": "ROLE",
      ...
      Model.observe('before save', function (instance, next) {
        ...
        if (data.syncToQB) {
          ...
          if (QBWebService.objectNotEmpty(data)) {
            ...
            if (instance.Model.definition.name == 'Customer') {
              if (!data.Name) {
            ...
            if (method == 'Add' && instance.Model.definition.name == 'Estimate') {
              data.EstimateLineAdd = data.EstimateLineMod;
            }
            if (method == 'Add' && instance.Model.definition.name == 'SalesOrder') {
          ...
          if (instance.Model.definition.name == 'Estimate') {
            let estimateLines = [];
          ...
          if (instance.Model.definition.name == 'SalesOrder') {
            let estimateLines = [];
          ...
          if (instance.Model.definition.name == 'Employee') {
            let extraDataFields = [];
            ...
            data.password = extraDataFields.filter(function (field) {
    ...
    if (transaction.method == 'Query') {
      ...
      if (transaction.modelName == 'SalesOrder' || transaction.modelName == 'Estimate') {
      ...
      transaction.operation = transaction.modelName.capitalize() + transaction.method;
  ...
  handleSuccessfulResponse(response, transaction) {
    ...
    function handleRecordData(recordData) {
      ...
      let Model = app.models[transaction.modelName];
      let ID = recordData.ListID || recordData.TxnID;
      if (!self.syncedData[transaction.modelName][ID]) {
        //record was just synced FROM QuickBooks and needs to be stored in Memory
        Model.create(recordData, function (err, res) {
          self.syncedData[transaction.modelName][ID] = res;
        });
      } else if (self.syncedData[transaction.modelName][ID].EditSequence != recordData.EditSequence) {
        let upsertWhereObj = recordData.ListID ? {ListID: ID} : {TxnID: ID};
        Model.upsertWithWhere(upsertWhereObj, recordData);
        self.syncedData[transaction.modelName][ID] = recordData;
      }
    }
...
// TODO: Is it ticket or wcTicket? (Mismatch in documentation)
  ...
  static buildXMLRequest(modelName, op, data, requestID) {
    ...
    var qbXML = xmlBuilder.create('QBXML', {version: '1.0'})
      .instruction('qbxml', 'version="13.0"');
    let qbXMLMsgsRq = qbXML.e('QBXMLMsgsRq').a('onError', 'stopOnError');
    var qbXMLOpRq = qbXMLMsgsRq.ele(modelName + op + 'Rq').a('requestID', requestID);
    var qbXMLOp = qbXMLOpRq.ele(modelName + op);
        ...
        if (data[propKey]) {
          if (modelProperties[propKey].constructor == Object) {
            if (QBWebService.objectNotEmpty(data[propKey])) {
              var qbXMLProperty = qbXMLOp.ele(propKey);
              ...
              for (var i2 = 0; i2 <= propertyCount2; i2++) {
                ...
                Object.keys(modelProperties2).forEach(function (propKey2) {
                  data[propKey][propKey2] && qbXMLProperty.ele(propKey2, data[propKey][propKey2])
                });
              }
            }
            ...
            data[propKey].forEach(function (item, arr, index) {
              if (QBWebService.objectNotEmpty(item)) {
                ...
                for (var i2 = 0; i2 <= propertyCount2; i2++) {
                  ...
                  console.log(modelProperties2)
                  Object.keys(modelProperties2).forEach(function (propKey2) {
                    if (item[propKey2]) {
                      if (modelProperties2[propKey2].constructor == Object) {
                        var qbXMLProperty3 = qbXMLProperty.ele(propKey2);
                        ...
                        for (var i3 = 0; i3 <= propertyCount3; i3++) {
                          ...
                          console.log(modelProperties3);
                          //   if (modelProperties.constructor == Object) {
                          //       var qbXMLProperty3 = qbXMLProperty2.ele(propKey);
                          //       var propertyCount3 = Object.keys(modelProperties2[propKey]).length - 1;
                          //       for ( var i2 = 0; i2 <= propertyCount2; i2++) {
                          //         var modelProperties2 = modelProperties[propKey][i2.toString()];
                          //       }
                          //   }
                          // Object.keys(modelProperties2).forEach(function(propKey2){
                          //   data[propKey][propKey2] && qbXMLProperty2.ele(propKey2, data[propKey][propKey2])
                          // });
                        }
                      } else {
                        qbXMLProperty.ele(propKey2, QBWebService.convertToProperDataType(modelProperties2[propKey], item[propKey2]))
                      }
                    }
                  });
                }
