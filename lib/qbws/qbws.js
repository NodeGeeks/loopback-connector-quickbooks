...
class QBWebService {
  ...
  reSync() {
    ...
    self.queueTransaction({modelName: 'Employee', method: 'Query'});
    self.queueTransaction({modelName: 'JobType', method: 'Query'});
    self.queueTransaction({modelName: 'Estimate', method: 'Query'});
    self.queueTransaction({modelName: 'Customer', method: 'Query'});
  ...
  queueTransaction(transaction) {
    ...
    if (!Model) {
      ...
      Model.observe('before save', function (instance, next) {
        ...
        let method = data.ListID && data.EditSequence ? 'Mod' : 'Add';
        if (data.syncToQB) {
          if (data.FirstName || data.LastName || data.MiddleName && !data.Name) {
            let names = [data.FirstName, data.MiddleName, data.LastName];
            data.Names = names.join(' ').replace('  ',' ');
          }
    ...
    if (transaction.method == 'Query') {
      ...
      for (let dataProperty in transaction.data) {
        ...
        if (transaction.data.hasOwnProperty(dataProperty)) {
          if (QBWebService.validatePropertyAvailability(dataProperty, transaction.modelName, transaction.method)) {
            if (transaction.data[dataProperty].constructor == Object) {
              ...
              for (let dataProperty2 in transaction.data[dataProperty]) {
                if (transaction.data[dataProperty].hasOwnProperty(dataProperty2)) {
                  if (QBWebService.validatePropertyAvailability([dataProperty, dataProperty2], transaction.modelName, transaction.method)) {
                    newDataObj[dataProperty] = QBWebService.validateCustom(transaction.data[dataProperty], [dataProperty, dataProperty2], transaction.modelName, transaction.method)
                    newDataObj[dataProperty][dataProperty2] = QBWebService.validatePropertyDataType([dataProperty, dataProperty2], transaction.modelName, transaction.method, transaction.data[dataProperty][dataProperty2]);
                  }
                }
              }
              if (Object.keys(newDataObj[dataProperty]).length == 0) {
                delete newDataObj[dataProperty];
              }
            } else {
              newDataObj[dataProperty] = QBWebService.validateCustom(transaction.data[dataProperty], dataProperty, transaction.modelName, transaction.method)
              newDataObj[dataProperty] = QBWebService.validatePropertyDataType(dataProperty, transaction.modelName, transaction.method, newDataObj[dataProperty]);
            }
          }
        }
      }
      let orderedDataObj = QBWebService.validatePropertyOrder(transaction.modelName, transaction.method, newDataObj);
...
// TODO: Is it ticket or wcTicket? (Mismatch in documentation)
  ...
  static validatePropertyDataType(dataProperty, modelName, method, val) {
    if (dataProperty.constructor == Array) {
  ...
  static validatePropertyOrder(modelName, method, data) {
