  ...
  queueTransaction(transaction) {
    ...
    if (!Model) {
      ...
      Model.observe('before save', function(instance, next){
        if (instance.isNewInstance && instance.instance.syncToQB) {
          //creating new record
          delete instance.instance.syncToQB;
          this.queueTransaction({method:'POST', data:instance.instance, modelName: instance.Model.definition.name})
        } else if (instance.instance.syncToQB){
          //updating existing record
          delete instance.instance.syncToQB;
          this.queueTransaction({method:'UPDATE', data:instance.instance, modelName: instance.Model.definition.name})
        }
        next();
      });
    }
  ...
  connectionError(args) {
    ...
    switch (hresult.trim()) {
      case QB_ERROR_WHEN_PARSING || QB_COULDNT_ACCESS_QB || QB_UNEXPECTED_ERROR:
        ...
        break;
      default :
        retVal = !self.errorCount ? '' && QBWebService.serviceLog('    Sending empty company file to try again.') : 'DONE' && QBWebService.serviceLog('    Sending DONE to stop.');
        break;
    }
